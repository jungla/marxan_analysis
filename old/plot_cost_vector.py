...
catch = pd.read_excel(fname, engine = 'openpyxl', sheet_name=1)

allcatch = trips.merge(catch,left_on='_index', right_on='_parent_index')
...
fishing_ground = fishing_ground.rename(columns={'Name':'Fishing_Trip/fishing_ground_name'})

fishing_effort = fishing_ground.merge(trips,on='Fishing_Trip/fishing_ground_name')

from shapely.ops import linemerge, unary_union, polygonize

fishing_ground.geometry.iloc[1]
b = fishing_ground.geometry.iloc[1].boundary.coords
linestrings = [LineString(b[k:k+2]) for k in range(len(b) - 1)]
print([list(ls.coords) for ls in linestrings])



lines = unary_union(linemerge([geom.exterior for shape in fishing_ground for geom in shape.geoms]))
